...
def generate_sample_ipv6_topology(num_stubs: int = 350) -> Tuple[Dict[int, ASNode], List[Tuple[int, int]]]:
    ...
    edges: List[Tuple[int, int]] = [
        (6939, 3356), (6939, 1299), (6939, 2914), (6939, 174), (6939, 6453), (6939, 3257),
        (3356, 174), (3356, 1299), (3356, 2914), (3356, 6453), (3356, 3257),
        (174, 1299), (174, 2914), (174, 6453), (174, 3257),
        (1299, 2914), (1299, 6762), (1299, 1273),
        (2914, 6453), (2914, 6762), (6453, 3257), (6762, 1273),
        (15169, 6939), (15169, 3356), (15169, 1299), (15169, 2914), (15169, 174),
        (13335, 6939), (13335, 174), (13335, 3356), (13335, 1299),
        (16509, 6939), (16509, 3356), (16509, 1299), (16509, 2914),
        (8075, 6939), (8075, 3356), (8075, 1299), (8075, 174),
        (20940, 6939), (20940, 3356), (20940, 174),
        (27699, 6939), (27699, 3356), (27699, 174), (27699, 1299),
        (2609, 6939), (2609, 3356), (2609, 27699),
        (37100, 6939), (37100, 1299), (37100, 6453), (37100, 3356),
        (36903, 6939), (36903, 1299), (36903, 37100),
    ]
    ...
    if file_path and os.path.exists(file_path):
        ...
        if country_code == "ID":
            ...
            for asn in country_asns:
                cone_sizes[asn] = 400 if asn == 7713 else (300 if asn == 4761 else (220 if asn == 24203 else (180 if asn == 64302 else int(np.random.exponential(12)))))
...
def main():
    parser = argparse.ArgumentParser(description="CAIDA IPv6 AS Core Visualizer (Global & Country-Level)")
    parser.add_argument("-i", "--input", help="Path to CAIDA IPv6 *.as-rel2.txt or *.as-rel2.txt.bz2 file", default=None)
    ...
    parser.add_argument("-n", "--top", help="Top N ASes to visualize by IPv6 customer cone", type=int, default=700)
    ...
    if output_filename is None:
        output_filename = f"as_core_ipv6_{country.lower()}.png" if country != "GLOBAL" else "as_core_ipv6_global.png"

    plot_title = args.title
    if plot_title is None:
        plot_title = f"{country} IPv6 AS Core Topology" if country != "GLOBAL" else "CAIDA IPv6 AS Core Topology"

...
def generate_sample_2020_topology(
    num_stubs: int = 350,
) -> tuple[dict[int, ASNode], list[tuple[int, int]]]:
...
def main():
    parser = argparse.ArgumentParser(
        description="CAIDA IPv4 AS Core Visualizer (Global & Country-Level)"
    )
    parser.add_argument(
        "-i",
        "--input",
        help="Path to CAIDA *.as-rel2.txt or *.as-rel2.txt.bz2 file",
        default=None,
    )
    parser.add_argument(
        "-c",
    ...
    parser.add_argument(
        "-n",
        "--top",
        help="Top N ASes to visualize by customer cone",
        type=int,
        default=700,
    )
    ...
    if output_filename is None:
        output_filename = (
            f"as_core_{country.lower()}.png" if country != "GLOBAL" else "as_core_global.png"
        )

    plot_title = args.title
    if plot_title is None:
        plot_title = (
            f"{country} IPv4 AS Core Topology"
            if country != "GLOBAL"
            else "CAIDA IPv4 AS Core Topology"
        )
